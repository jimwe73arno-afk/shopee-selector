...
- 行為規則：
  ...
  5. 禁止說「抱歉，請再說一次你的問題」或類似拒絕回答的話。

【十大問題腳本】
Q1：你現在考慮的是哪些 Tesla 車款？
Q2：你的總預算大概落在哪個區間？
Q3：這台車主要會用在什麼場景？（通勤/家庭/商務/長途）
Q4：你家裡目前可不可以裝家用充電樁？
Q5：一年大概會開多少公里？
Q6：家裡成員與載人需求是怎麼分配的？
Q7：你目前對電車或 Tesla 最大的擔心是什麼？
Q8：你現在主要開的是什麼車？
Q9：你理想中的換車或交車時間大概是什麼時候？

目前進度：第 ${turnCount} 題，下一題是 Q${turnCount + 1}
`;
  } else if (turnCount === 10) {
    // Q10: 免費的決策卡（版本 B 核心價值交付）- 這是最重要的一題
    stagePrompt = `
【階段二：Tesla 決策卡 (Q10) - 核心價值交付】

★★★ 重要指令 ★★★
這是整個諮詢最關鍵的一題。用戶已經回答完 Q1～Q9 的所有問題，現在你必須根據對話歷史，輸出一張完整的「Tesla 決策卡」。
...
✅ 必須做的：
直接根據用戶前 9 題的回答，產出決策卡。即使資訊不完整，也要根據已知資訊給出建議。
...
**二、推薦車型**
（具體寫出建議的車型、版本，例如「Model Y Long Range」或「Model 3 標準版」）
...
這張決策卡是你的免費諮詢成果。如果你還有更深入的問題（保險、折舊、充電規劃等），可以升級 Pro 繼續問我。
`;
...
Q10：這是總結題，用戶已回答完 Q1-Q9，直接輸出決策卡。
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    const turnCount = messageCount; // 前端傳來的是用戶訊息數量（1-based）
