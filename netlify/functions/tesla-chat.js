...
function getTeslaPrompt(userPlan, turnCount) {
  ...
  if (turnCount <= 9) {
    ...
    stagePrompt = `
【階段一：基本盤點 (Q1-Q9)】
- 任務：由你主動依序提問 Q1～Q9，不要跳題、不要改寫題意。
- 行為規則：
  1. 每一輪只問一題，對方回答完再換下一題。
  2. 開頭先用 1 句話肯定他的回答或情境（「這預算很 OK」「家裡能裝家充是大優勢」）。
  3. 然後清楚地丟出下一題腳本中的原文問題（Q${turnCount + 1}）。
  4. 不要給最終車型建議，不要分析太多，只要讓他舒服地把資訊講完。
- 回覆格式示例：
  「了解，這樣我對你的狀況又多了一點畫面，這條路線很符合一般 Tesla 車主的使用方式。
   那我想再問你一個關鍵點：Q${turnCount + 1}：…（照腳本內容原文丟出來）」 
`;
  } else if (turnCount === 10) {
    // Q10: 決策卡（用戶已看過廣告或付費才會到這裡）
    stagePrompt = `
【階段二：Tesla 決策卡 (Q10)】
- 前提：用戶已經回答完 Q1～Q9 的所有關鍵問題，且已解鎖此步驟。
- 任務：產出一張「Tesla 決策卡」，這是整個諮詢的核心價值交付。
- 輸出結構（務必按照此格式）：

### 🎯 你的 Tesla 決策卡

**一、結論**
（買不買？買哪台？何時買？用 2-3 句話講清楚）

**二、推薦車型**
（具體寫出建議的車型、版本，例如「Model Y Long Range」）

**三、兩個關鍵依據**
1. （從預算、家充、里程、家庭需求中挑最關鍵的）
2. （第二個關鍵理由）

**四、風險提醒**
（提醒 1-2 個真實可能踩雷的點，並給避坑方案）
...
🚗 **下訂福利**：如果你最後決定下訂，記得用這個推薦連結拿官方積分：https://ts.la/arno873937

💡 **BrotherG 生態系**：牽車後想去日本自駕？可以用 BrotherG Travel 幫你排行程；想買車主周邊配件，用 Shopee Analyst 幫你挑高 CP 值商品。

---

如果你還有任何問題，可以繼續問我，我會陪你走完整個購車決策。
`;
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    const { messages = [], intakeAnswers = [], messageCount = 0, paid = false, hasWatchedAdToday = false } = body;
    ...
    // 判斷用戶是否已解鎖（付費 OR 今天看過廣告）
    const isUnlocked = paid || hasWatchedAdToday;
    const userPlan = paid ? 'pro' : (hasWatchedAdToday ? 'ad_unlocked' : 'free');
    const turnCount = messageCount; // 前端傳來的是用戶訊息數量

    // ★ Q10 Gate 邏輯：
    // Q1-Q9: 免費
    // Q10: 需要解鎖（付費 OR 看廣告）才能看到決策卡
    // Q11+: 需要解鎖才能繼續深度諮詢
    
    if (turnCount >= 10 && !isUnlocked) {
      // Q10+: 需要解鎖
      // 回傳 Gate 提示（但不阻擋，前端會處理 Gate）
      return {
        statusCode: 200,
        headers,
        body: JSON.stringify({ 
